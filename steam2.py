...
class SteamProfileHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == '/login':
            ...
            if claimed_id:
                ...
                self.end_headers()
                self.wfile.write(b'Erro na autentica\xc3\xa7\xc3\xa3o.')
            ...
            steam_id = [cookie.split('=')[1] for cookie in cookies.split('; ') if cookie.startswith('steam_id')][0]
    ...
    def generate_profile_page(self, profile, games, cs_go_stats):
        ...
        games_info = [
            {'name': game.get('name', f'AppID {game["appid"]}'), 'playtime_hours': game['playtime_forever'] / 60} for
            game in games]
...
def run(server_class=HTTPServer, handler_class=SteamProfileHandler, port=8080):
    server_address = ('', port)
